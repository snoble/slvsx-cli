...
impl Translator {
    ...
    fn validate_references(&self, doc: &InputDocument) -> Result<()> {
        let entity_ids: std::collections::HashSet<_> = doc.entities.iter()
            .map(|e| e.id())
            .collect();
    ...
    fn validate_constraint_refs(&self, constraint: &Constraint, entity_ids: &std::collections::HashSet<&str>) -> Result<()> {
    ...
    fn get_constraint_refs(&self, constraint: &Constraint) -> Vec<String> {
        match constraint {
            ...
            Constraint::Angle { between, .. } => between.clone(),
            ...
            Constraint::PointOnCircle { point, circle: line } => vec![point.clone(), line.clone()],
            Constraint::Mesh { gear1, gear2 } => vec![gear1.clone(), gear2.clone()],
        }
    }
}
...
mod tests {
    ...
    fn test_validate_references_valid() {
        ...
        let doc = InputDocument {
            ...
            entities: vec![
                ...
                Entity::Line { id: "l1".to_string(), p1: "p1".to_string(), p2: "p2".to_string() },
            ],
            constraints: vec![
    ...
    fn test_validate_references_invalid() {
        ...
        let doc = InputDocument {
            ...
            constraints: vec![
                Constraint::Horizontal { a: "l1".to_string() }, // l1 doesn't exist
            ],
        };
        ...
        match result.unwrap_err() {
            Error::InvalidInput { message, .. } => assert!(message.contains("Unknown entity reference")),
    ...
    fn test_get_constraint_refs() {
        ...
        let constraint = Constraint::Horizontal { a: "l1".to_string() };
        assert_eq!(translator.get_constraint_refs(&constraint), vec!["l1"]);
        
        let constraint = Constraint::Perpendicular { a: "l1".to_string(), b: "l2".to_string() };
        assert_eq!(translator.get_constraint_refs(&constraint), vec!["l1", "l2"]);
        
        let constraint = Constraint::Coincident { at: "p1".to_string(), of: vec!["l1".to_string(), "l2".to_string()] };
        assert_eq!(translator.get_constraint_refs(&constraint), vec!["p1", "l1", "l2"]);
    }
}