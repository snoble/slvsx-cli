...
impl SchemaValidator {
    ...
    pub fn validate_json(&self, json: &Value) -> Result<()> {
        ...
        if !obj.contains_key("schema") {
            return Err(Error::SchemaValidation("Missing required field: schema".into()));
        }
        if !obj.contains_key("entities") {
            return Err(Error::SchemaValidation("Missing required field: entities".into()));
        }
        if !obj.contains_key("constraints") {
            return Err(Error::SchemaValidation("Missing required field: constraints".into()));
        }
        
        // Validate schema version
        if let Some(schema) = obj.get("schema").and_then(Value::as_str) {
            if schema != "slvs-json/1" {
                return Err(Error::SchemaValidation(format!("Invalid schema version: {}", schema)));
            }
        }
